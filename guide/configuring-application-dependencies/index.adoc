...
Now, let’s write a test using Testcontainers postgres module.
...
static PostgreSQLContainer<?> postgres =
       new PostgreSQLContainer<>("postgres:15.2-alpine")
               .withClasspathResourceMapping(
                       "init-db.sql", "/docker-entrypoint-initdb.d/", BindMode.READ_ONLY
                );
----
...
static PostgreSQLContainer<?> postgres =
       new PostgreSQLContainer<>("postgres:15.2-alpine")
        .withCopyFileToContainer(MountableFile.forHostPath(
                "/host/path/to/init-db.sql"), "/docker-entrypoint-initdb.d/"
        );
----
...
[source,java]
----
include::{codebase}/src/test/java/com/testcontainers/demo/LocalStackTest.java[lines="22..45,53..70"]
----
...
[source,java]
----
Container.ExecResult execResult = localStack.execInContainer("awslocal", "s3", "ls");
